...
function getPythonPath() {
  // Allow overriding in backend/.env
  if (process.env.PYTHON_PATH && process.env.PYTHON_PATH.trim()) {
    return process.env.PYTHON_PATH.trim();
  }
  // Windows typically provides the `py` launcher, while mac/linux often use python3
...
/* ================= MIDDLEWARE ================= */
// DEV CORS: allow any origin so Flutter web (random localhost port) can call the API.
...
/* ================= DATABASE SETUP ================= */
const setupDatabase = () => {
  // Ensure DB operations run in order so tables exist before inserts
  db.serialize(() => {
    db.run(`
